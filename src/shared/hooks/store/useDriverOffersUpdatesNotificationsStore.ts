...
export type TDriverOffersUpdatesSource = "offer" | "update";
export type TSortValue = "created-desc" | "created-asc" | "title-asc" | "title-desc";

export interface IDriverOffersUpdatesRow {
    id: number | null;
    row_key: string;
    campaign_id: string;
    source: TDriverOffersUpdatesSource;
    title: string;
    description: string;
...
interface DriverOffersUpdatesNotificationsState {
    rows: IDriverOffersUpdatesRow[];
    meta: IMeta;
    loading: boolean;
    error: string | null;
...
const useDriverOffersUpdatesNotificationsStore = create<DriverOffersUpdatesNotificationsState>((set) => ({
    rows: [],
    ...
    fetchNotifications: async () => {
        ...
        try {
            const [offersRaw, updatesRaw] = await Promise.allSettled([
                fetchAllPaged("/dashboard/driver-offers-notifications/campaigns"),
                fetchAllPaged("/dashboard/driver-updates-notifications/campaigns"),
            ]);

            const offersRows: IDriverOffersUpdatesRow[] = offersRaw.status === "fulfilled"
                ? offersRaw.value.map((item: any) => ({
                    id: item.latest_notification_id ?? null,
                    row_key: `driver-offer-${item.campaign_id}`,
                    campaign_id: item.campaign_id,
                    source: "offer" as const,
                    title: item.title,
                    description: item.description,
                    sent_drivers_count: Number(item.sent_drivers_count ?? 0),
            ...
            const updatesRows: IDriverOffersUpdatesRow[] = updatesRaw.status === "fulfilled"
                ? updatesRaw.value.map((item: any) => ({
                    id: item.latest_notification_id ?? null,
                    row_key: `driver-update-${item.campaign_id}`,
                    campaign_id: item.campaign_id,
            ...
            set({ error: extractErrorMessage(error, "Failed to fetch driver offers & updates notifications") });
        } finally {
            set({ loading: false });
        }
    },
...
export default useDriverOffersUpdatesNotificationsStore;
