...
const Pendingverifications = () => {
    ...
    return (
        <div className={clsx("space-y-4", "bg-main-white border border-main-whiteMarble common-rounded p-6 overflow-hidden")}>
            <ChartHeader title={t("pendingVerificationsTitle")} no={count} />
...
const Item = ({ item }: { item: IPendingVerification }) => {
    ...
    const at = formatAppDateTime(item.createdAt, i18n.language, "—");

    return (
        <div className="py-3 px-2 flex items-center justify-between bg-main-luxuryWhite common-rounded">
            <div className="flex items-center gap-3">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-main-sharkGray text-sm font-normal leading-[20px]">{t(`roles.${item.roleKey}`)}</p>
                </div>
            </div>
            <div className="gap-y-1 flex items-end justify-end flex-col">
                <p className="text-[12px] leading-[16px] text-main-silverSteel">{at}</p>
                <Link to="/verification" className="text-main-primary font-medium text-sm leading-[20px] capitalize">
                    {t("review")}
                </Link>
            </div>
        </div>
